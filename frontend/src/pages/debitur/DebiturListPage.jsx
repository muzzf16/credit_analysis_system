  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold">Data Debitur</h1>
          <p className="text-sm text-slate-400">{total} debitur terdaftar</p>
        </div>
        {['ADMIN', 'AO'].includes(user?.role) && (
          <button onClick={() => navigate('/debitur/tambah')} className="btn-primary">
      ...
      <div className="card p-0 overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan="8" className="table-cell text-center py-12"><div className="w-6 h-6 border-2 border-gold border-t-transparent rounded-full animate-spin mx-auto" /></td></tr>
              ) : data.length === 0 ? (
                ...
                <tr key={d.id} className="hover:bg-white/5 transition-colors">
                  ...
                  <td className="table-cell">
                    <div className="flex items-center gap-1">
                      <button onClick={() => navigate(`/debitur/${d.id}`)} className="btn-ghost p-1.5" title="Detail">
                        <Eye className="w-4 h-4" />
                      </button>
                      <button onClick={() => navigate(`/debitur/${d.id}/edit`)} className="btn-ghost p-1.5" title="Edit">
                        <Edit className="w-4 h-4" />
                      </button>
                      <button onClick={() => navigate(`/pengajuan/tambah?debiturId=${d.id}`)} className="btn-ghost p-1.5 text-gold" title="Buat Pengajuan">
