...
const FactDefinition = require('../entities/FactDefinition');
...
class DefinitionRegistry {
  ...
  register(definitionEntity) {
    if (!(definitionEntity instanceof FactDefinition)) {
      throw new Error('Can only register instances of FactDefinition');
    }
    
    if (this._definitions.has(definitionEntity.code)) {
      throw new Error(`FactDefinition [${definitionEntity.code}] is already registered.`);
    }
  ...
  loadFromDirectory(dirPath) {
    ...
    for (const file of files) {
      ...
      const def = new FactDefinition(rawData);
      this.register(def);
    }
  }
