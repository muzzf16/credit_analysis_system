...
async function saveKonsumtif(data, userId) {
  ...
  const result = await db.query(
    `INSERT INTO analisa_konsumtif (pengajuan_id,
     ...
     listrik, air, transportasi, pendidikan, cicilan_existing, kebutuhan_rumah_tangga, pengeluaran_lain, total_pengeluaran,
    ...
    [pengajuanId,
     ...
     data.listrik || 0, data.air || 0, data.transportasi || 0, data.pendidikan || 0, data.cicilanExisting || 0, data.kebutuhanRumahTangga || 0, data.pengeluaranLain || 0, calc.totalPengeluaran,
...
async function saveProduktif(data, userId) {
  ...
  const result = await db.query(
    `INSERT INTO analisa_produktif (pengajuan_id,
     ...
     dscr, working_capital, break_even_point,
    ...
    [pengajuanId,
     ...
     calc.dscr, calc.workingCapital, calc.breakEvenPoint,
